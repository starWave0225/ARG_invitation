...
test("shows the B-17 arrival mail notification only once per playthrough", async () => {
  ...
  assert.match(desktop,/localStorage\.getItem\("jia-notified-b17-arrival"\)!=="true"/);
  assert.match(desktop,/const b17AlreadyHandled=localStorage\.getItem\("jia-gupan-pc-unlocked"\)==="true";/);
  assert.match(desktop,/if\(owner==="shen"&&b17AlreadyHandled\)localStorage\.setItem\("jia-notified-b17-arrival","true"\);/);
  assert.match(desktop,/if\(!notice\|\|wechatNotice\|\|owner!=="shen"\)return;/);
  assert.match(desktop,/localStorage\.setItem\("jia-notified-b17-arrival","true"\);/);
