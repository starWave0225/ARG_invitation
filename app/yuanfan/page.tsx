...
export default function YuanfanSite(){
  ...
  return <main className="route-page yuanfan-route"><div className="aid-public">
    ...
    <section className={`aid-site-tools ${hasAccess?"unlocked":"locked"}`}>
      <div><small>MEMBER SITE ACCESS</small><h3>{hasAccess===null?"正在读取访问状态…":hasAccess?"学生网站权限已开通":"此区域仅对已核验学生开放"}</h3><p>{hasAccess===null?"请稍候，正在确认你的学生访问状态。":hasAccess?"成员公告、活动日历、新生指南与站内搜索已经启用。":"请通过远帆联络人完成学生身份核验。"}</p></div>
      <form onSubmit={search}><label htmlFor="yuanfan-search">站内搜索</label><div><input id="yuanfan-search" value={query} disabled={!hasAccess} onChange={event=>{setQuery(event.target.value);setSearchError(false)}} placeholder={hasAccess===null?"正在读取访问状态…":hasAccess?"搜索公告、项目或关键词":"需要学生网站权限"}/><button disabled={!hasAccess}>搜索</button></div>{searchError&&<p>没有找到与“{query}”匹配的公开内容。</p>}{searchAttempts>=3&&<p className="aid-search-warning">你到底是谁？你想知道什么？</p>}</form>
    </section>
