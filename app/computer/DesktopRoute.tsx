...
export default function DesktopRoute({owner}:{owner:Owner}){
  ...
  return <main className={`pc-desktop pc-${owner}`}>
    ...
    {mobileDevicesOpen&&<nav className="pc-mobile-device-menu" aria-label="切换电脑">
      ...
      <p className="pc-mobile-system-time">{cfg.date} · {systemTime}</p>
      {gameMode&&<button type="button" onClick={toggleGameMode}><span>{gameMode==="normal"?"通灵模式":"真实模式"}</span><small>切换为{gameMode==="normal"?"真实模式":"通灵模式"} ⇄</small></button>}
      <a href="/"><span>返回主选单</span><small>保留当前进度 ›</small></a>
      <button type="button" onClick={resetGame}><span>↻ 重置进度</span><small>确认后重新开始</small></button>
    </nav>}
...
function PcWindow({owner,app,gameMode,close}:{owner:Owner;app:string;gameMode:GameMode|null;close:()=>void}){
  ...
  useEffect(()=>setImageZoomed(false),[previewImage]);
...
function FilePreview({owner,file,close}:{owner:Owner;file:string;close:()=>void}){
  const content:Record<string,{tag:string;title:string;body:React.ReactNode;link?:[string,string]}>={
    ...
    "07_现场物品登记表.pdf":{tag:"临川公安 · 现场物品",title:"现场物品登记表",body:<><table className="scene-evidence-table"><thead><tr><th>编号</th><th>物品</th><th>发现位置</th><th>状态</th></tr></thead><tbody><tr><td>LC-402-04</td><td>碎屏旧手机</td><td>卧室窗边矮柜后侧</td><td>关机 · 无SIM卡</td></tr><tr><td>LC-402-05</td><td>婚礼请柬</td><td>书桌抽屉第二层</td><td>纸面完整 · 二维码可识别</td></tr><tr><td>LC-402-06</td><td>婚庆合同</td><td>碎纸篓夹层</td><td>纸张残损 · 封边校验清晰</td></tr><tr><td>LC-402-09</td><td>酒红色长围巾</td><td>室内悬挂点下方</td><td>已单独封存</td></tr></tbody></table><p>登记表仅列出与本次线索核验直接相关的现场物品。原物继续由办案单位封存。</p><a className="scene-download" href="data:text/plain;charset=utf-8," download="LC-QH-1129-402.txt" onClick={()=>{localStorage.setItem("jia-police-investigation-id-downloaded","true");window.dispatchEvent(new Event("jia-progress"))}}>下载附件</a></>},
  };
