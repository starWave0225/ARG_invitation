...
export default function ShenWangWeibo(){
  return <main className="wb-route zw-wb">
    ...
    <div className="wb-layout">
      ...
      <section className="wb-feed"><div className="wb-filter"><b>他的微博</b><span>按时间排序</span></div>{posts.map((post,index)=><article key={post.date}><img src="/characters/shen-wang.png" alt="沈望"/><div><header><b>向左望，向右看</b><small>{post.date}　{post.note}</small></header><p>{post.text}</p>{index===3&&<div className="wb-attachment">旧照片 · IMG_20181021_ARTSHOW.jpg<small>图片因原图失效暂不可查看</small></div>}<footer><span>☆ 收藏</span><span>↗ 转发</span><span>□ 评论</span><span>♡ 赞</span></footer></div></article>)}</section>
